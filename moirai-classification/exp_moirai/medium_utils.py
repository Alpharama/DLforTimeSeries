...
def generate_model_input(
    ...
    if median_forecast is not None:
        ...
        median_forecast_ts = pd.DataFrame(
            {
                date_colname: pd.date_range(
                    last_date_train + frequency,
...
def get_predictions(
    ...
    date_colname: str,
    percentile_inf: int = 5,
    percentile_sup: int = 95,
):
    """
...
def plot_results(
    results_df: pd.DataFrame, target_colname: str, title: str, figsize=(7, 6)
):
    ...
    ax.plot(
        results_df.index,
        results_df["median_forecast"].values,
        "g-",
        label="median forecast",
    )
    ax.plot(
        results_df.index, results_df[target_colname].values, "b-", label=target_colname
    )
    ax.fill_between(
        results_df.index,
