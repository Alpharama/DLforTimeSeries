...
class MeanPoolingClassifier(nn.Module):
    """
    Classifier that aggregates patch embeddings using mean pooling for each variable.

    The pooled variable representations are flattened and passed through a
...
class SingleScaleAttentionClassifier(nn.Module):
    """
    Classifier that learns attention weights over patches for each variable.

    A shared or variable-specific context vector is used to pool patch
...
class SingleScaleMultiHeadClassifier(nn.Module):
    """
    Classifier that applies multi-head attention over patches for each variable.

    A learned classification token attends to patch embeddings to produce
    variable-level representations for classification.
    """
...
class HierarchicalMultiHeadClassifier(nn.Module):
    """
    Hierarchical classifier that first attends over patches and then over variables.

    Multi-head attention is used to build variable-level representations and
    then combine them into a global representation for classification.
    """
